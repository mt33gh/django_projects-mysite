...
def failform(request):  # this is basically the same as postform except for the csrf decoration 
...
def csrfform(request):  # this is a version of failform with a CSRF token
    ...
    token = get_token(request)  # get a token and put it into "token"
    response = response.replace('__token__', html.escape(token)) #the __token__ part is replaced by the escaped token
    response += dumpdata('POST', request.POST)
    return HttpResponse(response) 
