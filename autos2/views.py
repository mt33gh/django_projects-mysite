...
from autos2.models import Auto, Make
from autos2.forms import MakeForm

# Create your views here.
# These views are original from the dj4e-sample code.
# However, I changed autos to be autos2.
...
class MainView(LoginRequiredMixin, View):
    def get(self, request):
        ...
        return render(request, 'autos2/auto_list.html', ctx)

class MakeView(LoginRequiredMixin, View):
    def get(self, request):
        ml = Make.objects.all()
        ctx = {'make_list': ml}
        return render(request, 'autos2/make_list.html', ctx)

# We use reverse_lazy() because we are in "constructor attribute" code
# that is run before urls.py is completely loaded
class MakeCreate(LoginRequiredMixin, View):
    template = 'autos2/make_form.html'
    success_url = reverse_lazy('autos2:all')
    def get(self, request):
        form = MakeForm()
        ctx = {'form': form}
...
class MakeDelete(LoginRequiredMixin, View):
    model = Make
    success_url = reverse_lazy('autos2:all')
    template = 'autos2/make_confirm_delete.html'
    def get(self, request, pk):
        make = get_object_or_404(self.model, pk=pk)
        ctx = {'make': make}
        return render(request, self.template, ctx)
